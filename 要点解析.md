![image-20210824103652399](https://images.yewq.top/image-20210824103652399.png)
...
function new_instance_of(leftVaule, rightVaule) { 
    if (typeof leftValue !== 'object' || typeof rightValue !== 'object') throw Error('Right-hand side of 'instanceof' is not an object');
...
- 老生代: 新生代中的对象存活一段时间会转移到老生代内存区. 老生代又分为指针区和数据区. 指针区包含大多可能存在指向其他对象的指针对象.数据区只保存原始数据对象. 采用`Mark-Sweep`和`Mark-Compact`算法. 标记采用增量标记概念,这个理念其实有点像`React`框架中的`Fiber`架构
...
**键对象的“弱引用", 这意味着在没有其他引用存在时垃圾回收能正确进行**, **key 是不可枚举的**
